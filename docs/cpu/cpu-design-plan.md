...
- COP 独立后端已经按 `409575a` 整理到 `vsrc/vector/cop`；
- `539bf41` 的 COP 连续/混合提交时序修复已同步到 CPU 侧；
- 当前最新已发布 CPU 稳定提交为 `cdedfd5 perf: attribute LSU start stalls`。
...
- `load hit` 当前拍完成：已经在 `vsrc/cpu/exu/lsu.v` 落地，并通过 `add`、`load-store`、`mem`、`quick-sort` 回归；
- `store hit` 当前拍完成：已经在 `vsrc/cpu/exu/lsu.v` 落地，并通过访存、排序和全量 CPU 回归；
