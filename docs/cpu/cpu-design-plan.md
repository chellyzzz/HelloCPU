...
### #2 Other backend（23% of stalls，A 线下一步）

`idu2exu_valid && exu2idu_ready && !exu2wbu_valid` — EXU 接受了指令但结果未到 WBU。疑似 MULH/MULHSU/MULHU 的 2-cycle 延迟（940K multiplies × ~1 cycle each ≈ 940K）。待分析。
...
Other backend 分析（MUL 延迟？）、store buffer、旁路强化。
