...
3. 至少支持一个明确的 RVV 子集，包括 `vsetvli`、unit-stride load/store、基础整数向量算术和全使能/简单 mask 执行。
...
1. 合入 pending-kill directed coverage，结束当前 COP memory owner routing 后续收敛。
2. 更新 `cop-encoding.md` 和 `vector-memory-access.md`，标清 custom prototype 与 RVV target 的边界。
3. 新增 `rvv-supported-subset.md` 空矩阵，先列计划支持项和 unsupported 项。
4. 设计最小 `vl/vtype` RTL 草案，只写文档和接口讨论，不急于改 CPU decode。
5. 等 CPU mainline 稳定后，从 `vsetvli`/`vsetivli` decode 开始做第一条 RVV path。
