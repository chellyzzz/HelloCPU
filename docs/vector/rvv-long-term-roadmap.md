...
- 建立 `sw/benchmark/rvv-subset-benchmark/` benchmark harness。当前已落地初版 build/run 入口和前三个 benchmark。
- 跑通 `vec_add_i32`、`vec_xor_u8`、`memcpy_vec`、`dot_i32_tiny`。初版使用 static initialized backing memory，`rvv_debug_*` 仅作为 harness 辅助。
- 已补 `vle16/vse16`、`vmul.vv/vx`、compare mask、`vmerge` 和 `vredsum.vs`。
- 已跑通 `threshold_u8` 本地 benchmark；后续重点是 coherency contract 变成 architectural acceptance 前的额外覆盖。
...
1. 使用 `make rvv-subset-smoke EXTRA_VERILATOR_FLAGS='-j 1'` 作为 partial RVV 固定回归入口。
2. 使用 `make rvv-bench-run EXTRA_VERILATOR_FLAGS='-j 1'` 运行当前 RVV benchmark harness。
3. 明确 scalar cache 与 COP memory bypass coherency contract，避免 benchmark 结果依赖 static backing memory。
4. 实现 `vsetvli`，让 benchmark loop 使用标准 AVL 配置形式。
5. 将当前 benchmark harness 从 static backing memory 过渡到可声明的 scalar/vector coherency acceptance。
...
- 能运行 `sw/benchmark/rvv-benchmark/` 中的目标 benchmark，并通过 scalar reference check。
