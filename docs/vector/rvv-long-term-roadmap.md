...
- 不以完整 RVV compliance 为最终目标；最终目标是支撑 `sw/benchmark/rvv-benchmark/` 的小型整数 benchmark。
- 不在语义稳定前追求多 lane 高吞吐。
...
目标：让 `sw/benchmark/rvv-benchmark/` 成为最终 partial RVV 的验收入口，而不是继续追完整 RVV。
...
- 建立 `sw/benchmark/rvv-benchmark/` benchmark harness。
- 跑通 `vec_add_i32`、`vec_xor_u8`、`memcpy_vec`。
- 补 `vsetvli`、`vle16/vse16`、`vmul.vv/vx`、compare mask、`vmerge`、`vredsum.vs`。
- 跑通 `dot_i32_tiny` 和 `threshold_u8`。
- 冻结 scalar/vector memory coherency contract。
...
1. 建立 `sw/benchmark/rvv-benchmark/` harness 和 scalar reference pattern。
2. 先补一键 smoke target，固定当前 partial RVV 回归集合。
3. 明确 scalar cache 与 COP memory bypass coherency contract，避免 benchmark 结果依赖偶然内存路径。
4. 实现 `vsetvli`，让 benchmark loop 使用标准 AVL 配置形式。
5. 按 benchmark 需求补 `vle16/vse16`、`vmul.vv/vx`、compare mask、`vmerge`、`vredsum.vs`。
