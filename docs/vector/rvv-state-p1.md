...
这两个编码仍属于 custom COP prototype。标准 OP-V `vsetivli`、unmasked `vadd.vv/vadd.vx` 和 unmasked bitwise VV 最小 slice 已复用相同状态语义；其他 OP-V decode、trap/illegal path、CSR 可见状态仍留给后续 review。
...
- 标准 OP-V `vsetivli`、unmasked `vadd.vv/vadd.vx` 和 unmasked bitwise VV 最小 slice 已验证；进入其他 OP-V decode 前 CPU decode/trap/CSR interface review 仍需完成。
