...
| File | Change |
|------|--------|
| `vsrc/exu/exu.v` | `predict_correct` now means any correctly predicted control flow, including not-taken branches |
| `vsrc/wbu/wbu.v` | Redirect decision uses `i_is_brch && !i_predict_correct`, not actual-taken branch output |
| `vsrc/top/hcpu.v` | Branch WBU redirect PC uses EXU `o_redirect_pc`, so not-taken recovery redirects to `pc + 4` |
