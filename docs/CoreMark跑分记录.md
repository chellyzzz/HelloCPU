...
> 测试日期: 2026-05-02 | CPU: hcpu (RV32IM + Zicsr, 5 级流水线) | 分支预测器: 2026-05-03
...
> 2026-05-03 分支预测器版本: 37/37 全部通过 (100%)

---

## 九、分支预测器性能 (2026-05-03)

### 整体测试结果

**37/37 全部通过 (100%)** — 分支预测器版本 (`branch-predictor`)

### 各测试性能对比

| 测试 | 原始周期 | 预测器周期 | 改善 | BTB hit% | RAS hit% |
|------|---------|-----------|------|----------|----------|
| quick-sort | 15,458 | 8,346 | **-46%** | 82.2% | 95.9% |
| recursion | 47,638 | 13,379 | **-72%** | 55.5% | 0% |
| bubble-sort | 13,878 | 10,515 | **-24%** | 94.1% | 40.0% |
| select-sort | 10,565 | 8,696 | **-18%** | 96.9% | 48.7% |
| pascal | 12,746 | 9,652 | **-24%** | 84.0% | 0% |
| string | 7,309 | 5,381 | **-26%** | 81.9% | 68.4% |
| prime | 51,997 | 47,162 | **-9%** | 88.9% | 0% |
| wanshu | 19,437 | 18,398 | **-5%** | 98.6% | 0% |
| unalign | 963 | 664 | **-31%** | 37.5% | 0% |
| hello | 394 | 334 | **-15%** | 91.7% | - |

### 预测器性能特点

| 指标 | 典型值 | 说明 |
|------|--------|------|
| BTB hit rate | 55-98% | 依赖分支重复模式 |
| BTB 学习延迟 | ~1 mispredict | 首次跳转后 BTB 分配条目 |
| RAS hit rate | 40-96% | 仅当编译器使用 ra(x1) 时 |
| JAL target mismatch | **0** | 100% 正确, 经验证 |

### CoreMark ITER=1 实测 (BTB-only, 2026-05-03)

由于 BTB+JAL 组合导致死循环, 本测试仅启用 BTB 预测 (禁用 JAL/RAS)。

```
2K performance run parameters for coremark.
CoreMark Size    : 666
Total ticks      : 794700
Total time (secs): 794700
Iterations/Sec   : 0
Iterations       : 1
Compiler version : riscv64-linux-gnu-gcc 11.4.0
...
Memory location  : STACK
seedcrc          : 0xe9f5
[0]crclist       : 0xc577   ← CRC ERROR (expected 0xe714)
[0]crcmatrix     : 0x1fd7   ✓
[0]crcstate      : 0x3310   ← CRC ERROR (expected 0x8e3a)
[0]crcfinal      : 0xc577   ← CRC ERROR (expected 0x988c)
Errors detected
Total cycles     : 875235
CoreMark/MHz     : 1.142
```
...
| 指标 | 数值 |
|------|------|
| 状态 | **CRC 错误** (crclist/crcstate/crcfinal 不匹配) |
| IPC | 0.449 |
| BTB hit | 65.1% |
| BTB mispredict | 30.5% |
| RAS hit | 90.4% |
...
> **⚠️ 已知问题**: BTB-only 下 CoreMark ITER=1 虽能跑完 (879,918 cycles), 但 **CRC 校验失败**。
> 这说明分支预测器存在数据正确性 bug, 虽然 37 个 cpu-tests 全部通过但 CoreMark 的严格 CRC 检查发现了错误。
>
> BTB+JAL 全预测模式会陷入死循环 (仅 170 次 JAL/JALR) 无法完成任何迭代。
